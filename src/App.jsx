...
// We use fetch directly but with proper session handling
...
const db = {
  ...
  async addGoal(goal) {
    await supabase.from('goals').insert({ id: goal.id, title: goal.title, description: goal.desc, target: goal.target, progress: goal.progress })
  },
  async updateGoal(id, fields) {
    await supabase.from('goals').update(fields).eq('id', id)
  },
  async deleteGoal(id) {
    await supabase.from('logs').delete().eq('goal_id', id)
    await supabase.from('goals').delete().eq('id', id)
  },
  async addLog(log, goalId) {
    await supabase.from('logs').insert({ id: log.id, goal_id: goalId, text: log.text,
...
// ── HELPERS ───────────────────────────────────────────────────
const scoreColor = s => s >= 70 ? '#3ecf8e' : s >= 40 ? '#f0954a' : '#e05c72'
...
function Btn({ onClick, style, disabled, children }) {
  return <button onClick={e => { e.stopPropagation(); if(!disabled) onClick&&onClick() }} disabled={disabled}
    style={{ WebkitTapHighlightColor:'transparent', touchAction:'manipulation', userSelect:'none', cursor:disabled?'default':'pointer', ...style }}>{children}</button>
}
...
const I = {
  ...
  Star:    () => <svg width="12" height="12" viewBox="0 0 24 24" fill="currentColor"><path d="M12 2l2.4 7.4H22l-6.2 4.5 2.4 7.4L12 17l-6.2 4.3 2.4-7.4L2 9.4h7.6z"/></svg>,
...
const S = {
  ...
  btnSm: (bg='transparent',color='#8888aa') => ({ display:'inline-flex', alignItems:'center', justifyContent:'center', width:44, height:44, borderRadius:8, border:'1px solid #252540', background:bg, color, WebkitTapHighlightColor:'transparent', touchAction:'manipulation', userSelect:'none', WebkitAppearance:'none' }),
...
const css = `*{box-sizing:border-box}body{margin:0;background:#080810}@keyframes spin{from{transform:rotate(0deg)}to{transform:rotate(360deg)}}@keyframes fadeUp{from{opacity:0;transform:translateY(14px)}to{opacity:1;transform:translateY(0)}}input,textarea,button{font-family:inherit}`

function Spinner() { return <span style={{ animation:'spin 1s linear infinite', display:'inline-block' }}>◌</span> }
    ...
    <div style={{ minHeight:'100vh', background:'#080810', color:'#dde1f5', display:'flex', alignItems:'center', justifyContent:'center', padding:24 }}>
      ...
      <div style={{ width:'100%', maxWidth:360, animation:'fadeUp 0.4s ease both' }}>
        <div style={{ textAlign:'center', marginBottom:40 }}>
          <div style={{ fontFamily:'Georgia,serif', fontWeight:700, fontSize:36, background:'linear-gradient(120deg,#dde1f5,#7c6ff7)', WebkitBackgroundClip:'text', WebkitTextFillColor:'transparent', marginBottom:10 }}>PROOF</div>
        ...
        <div style={{ background:'#0e0e1c', border:'1px solid #1a1a30', borderRadius:16, padding:'28px 24px' }}>
          <p style={{ color:'#8888aa', fontSize:13, lineHeight:1.8, textAlign:'center', margin:'0 0 24px' }}>记录你的成长，让 AI 严格评估每一步进展。</p>
          {error && <div style={{ color:'#e05c72', fontSize:12, marginBottom:12, padding:'8px 12px', background:'#1a0a0a', borderRadius:6 }}>{error}</div>}
          <Btn style={{ ...S.btn('#fff','#1a1a2a'), width:'100%', fontSize:14, gap:10, border:'1px solid #e0e0e0' }} onClick={handleGoogle} disabled={loading}>
            {loading ? <><Spinner /> 跳转中...</> : <><I.Google /> 使用 Google 账号登录</>}
          </Btn>
          <p style={{ ...S.mono, fontSize:10, color:'#333350', textAlign:'center', marginTop:16, lineHeight:1.8 }}>登录即表示你的数据将安全存储在云端<br/>仅你本人可见</p>
        </div>
      </div>
    </div>
  )
...
export default function App() {
  ...
  // Auth state listener - handles login, logout, and OAuth redirects automatically
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    setDataLoading(true)
    db.loadGoals()
      .then(data => { setGoals(data); setDataLoading(false) })
      .catch(() => { setDataLoading(false); setSyncStatus('error') })
  }, [user])
  ...
  const saveEditGoal = async () => {
    ...
    setGoals(p => p.map(g => g.id!==id ? g : {...g, title:title.trim(), desc:desc.trim()}))
    setEditingGoal(null)
    await withSync(() => db.updateGoal(id, { title:title.trim(), description:desc.trim() }))
  }

  const deleteGoal = async id => {
    setGoals(p => p.filter(g => g.id!==id)); setView('home')
    await withSync(() => db.deleteGoal(id))
  }

  const addLog = async () => {
    if (!logText.trim()) return
    const goal = goals.find(g => g.id===activeId); if (!goal) return
    setEvaluating(true); setEvalError('')
    let ai
    try { ai = await getAIEvaluation(goal.title, goal.desc, logText) }
    catch(e) { setEvalError(e.message==='NO_KEY' ? '⚠️ 未配置 API Key，请在 Vercel 添加 VITE_ANTHROPIC_KEY。' : 'AI评估失败，请重试。'); setEvaluating(false); return }
    ...
    setGoals(p => p.map(g => g.id!==activeId ? g : {...g, logs:[log,...g.logs], progress:newProgress}))
  ...
  const deleteLog = async (goalId, logId) => {
    ...
    setGoals(p => p.map(g => g.id!==goalId ? g : {...g, logs:newLogs, progress:newProgress}))
  ...
  if (authLoading) return (
    <div style={{ minHeight:'100vh', background:'#080810', display:'flex', alignItems:'center', justifyContent:'center' }}>
      <style>{css}</style>
      <span style={{ color:'#555570', fontFamily:'DM Mono,monospace', fontSize:13, display:'flex', alignItems:'center', gap:8 }}><Spinner /> 加载中...</span>
    </div>
  )
  ...
  if (!supabase) return (
    <div style={{ minHeight:'100vh', background:'#080810', display:'flex', alignItems:'center', justifyContent:'center' }}>
      <style>{css}</style>
      <span style={{ color:'#e05c72', fontFamily:'DM Mono,monospace', fontSize:13 }}>⚠️ Supabase SDK 未加载，请检查 index.html</span>
    </div>
  )
  ...
  const syncUI = { synced:{color:'#3ecf8e',text:'✓'}, syncing:{color:'#f0954a',text:'↑'}, error:{color:'#e05c72',text:'✗'} }[syncStatus]
  const avatar = user.email?.[0]?.toUpperCase() || '?'

  const XBtn = ({ onClick }) => <Btn style={{ background:'none', border:'none', color:'#555570', fontSize:24, width:44, height:44, display:'flex', alignItems:'center', justifyContent:'center' }} onClick={onClick}>×</Btn>
  ...
  if (view==='detail' && activeGoal) {
    ...
    return (
      <div style={{ minHeight:'100vh', background:'#080810', color:'#dde1f5' }}>
        <style>{css}</style>
        <div style={S.topbar}>
          <Btn onClick={() => setView('home')} style={{ background:'none', border:'none', color:'#8888aa', display:'flex', alignItems:'center', gap:6, fontSize:13, minHeight:44, padding:'0 8px 0 0' }}><I.Back /> 返回</Btn>
          <span style={S.logo}>PROOF</span>
          <div style={{ display:'flex', alignItems:'center', gap:8 }}>
            <span style={{ ...S.mono, fontSize:12, color:syncUI.color }}>{syncUI.text}</span>
            <Btn style={{ ...S.btnSm('#1a1a2e','#e05c72'), borderColor:'#2a1a1a' }} onClick={() => confirm('确认删除？') && deleteGoal(activeGoal.id)}><I.Trash /></Btn>
          </div>
        </div>
        <div style={S.page}>
          <div style={{ marginBottom:20 }}>
            ...
            {activeGoal.desc ? <p style={{ color:'#8888aa', fontSize:13, lineHeight:1.8, margin:'0 0 16px' }}>{activeGoal.desc}</p>
            ...
            <div style={{ background:'#0e0e1c', border:'1px solid #1a1a30', borderRadius:14, padding:'16px 18px' }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-end', marginBottom:10 }}>
                <div><div style={{ ...S.label, marginBottom:2 }}>总进度</div><div style={{ fontSize:36, fontWeight:800, color:pc, ...S.mono, lineHeight:1 }}>{pct}%</div></div>
                <div style={{ textAlign:'right' }}><div style={{ ...S.label, marginBottom:2 }}>累计得分</div><div style={{ fontSize:20, fontWeight:700, color:'#8888aa', ...S.mono }}>{activeGoal.progress}<span style={{ fontSize:12, color:'#333350' }}>/{activeGoal.target}</span></div></div>
              </div>
              ...
              <div style={{ display:'flex', justifyContent:'space-between', marginTop:8 }}>
                ...
                <span style={{ ...S.mono, fontSize:11, color:'#444460' }}>目标 {activeGoal.target} 分</span>
              </div>
            </div>
          </div>
          <Btn style={{ ...S.btn(), width:'100%', marginBottom:20 }} onClick={() => { setShowAddLog(true); setEvalError('') }}><I.Plus /> 记录今天的进展</Btn>
              ...
              <div key={log.id} style={{ background:'#0a0a14', border:'1px solid #16162a', borderRadius:11, padding:'13px 14px', marginBottom:8 }}>
                <div style={{ display:'flex', gap:10 }}>
                  ...
                  <div style={{ display:'flex', flexDirection:'column', gap:6, flexShrink:0 }}>
                    ...
                    <Btn style={{ ...S.btnSm('transparent','#e05c72'), borderColor:'#2a1a1a' }} onClick={() => confirm('删除？') && deleteLog(activeGoal.id, log.id)}><I.Trash /></Btn>
                  </div>
                </div>
                {exp && (
          ...
          <div style={S.overlay}>
            <div style={S.sheet}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
                ...
                {!evaluating && <XBtn onClick={() => setShowAddLog(false)} />}
              </div>
              ...
              <Btn style={{ ...S.btn(evaluating||!logText.trim()?'#1a1a2e':'#7c6ff7'), width:'100%', marginTop:12, opacity:evaluating||!logText.trim()?0.5:1 }} onClick={addLog} disabled={evaluating||!logText.trim()}>
                {evaluating ? <><Spinner />&nbsp;AI 评估中...</> : <><I.Star />&nbsp;提交并获得 AI 评估</>}
              </Btn>
            </div>
          </div>
        )}
          ...
          <div style={S.overlay}>
            <div style={S.sheet}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:16 }}>
                ...
                <XBtn onClick={() => setEditingGoal(null)} />
              </div>
    ...
    <div style={{ minHeight:'100vh', background:'#080810', color:'#dde1f5' }}>
      ...
      <div style={S.topbar}>
        ...
        <div style={{ display:'flex', alignItems:'center', gap:8 }}>
          ...
          <div style={{ width:34, height:34, borderRadius:'50%', background:'#7c6ff7', display:'flex', alignItems:'center', justifyContent:'center', fontSize:13, fontWeight:700, color:'#fff', cursor:'pointer', flexShrink:0 }}
            onClick={() => confirm(`退出登录？\n${user.email}`) && handleSignOut()}>
            {avatar}
          </div>
        </div>
      </div>
      <div style={S.page}>
        ...
        {dataLoading && <div style={{ textAlign:'center', padding:'48px 0', color:'#333350', ...S.mono, fontSize:13 }}><Spinner /> 加载中...</div>}
        ...
        <div style={S.overlay}>
          <div style={S.sheet}>
            <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:18 }}>
              ...
              <XBtn onClick={() => setShowAddGoal(false)} />
            </div>
            <div style={{ display:'flex', flexDirection:'column', gap:14 }}>
